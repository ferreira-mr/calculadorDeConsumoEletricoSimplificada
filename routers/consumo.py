...
def calcular_consumo_endpoint(
    ...
    if origem_do_consumo == EnumOrigemDoConsumo.dispositivo_eletrico:
        dispositivo_eletrico = DispositivoEletrico.get_or_none(DispositivoEletrico.id == item_id)
        ...
        dispositivos_eletricos = list(DispositivoEletrico.select().where(DispositivoEletrico.comodo == comodo))
        ...
        dispositivos_eletricos = list(DispositivoEletrico.select().where(DispositivoEletrico.residencia == residencia))

    consumo_diario, consumo_mensal, consumo_anual = calcular_consumo(dispositivos_eletricos)

    return ConsumoRead(consumo_diario=consumo_diario, consumo_mensal=consumo_mensal, consumo_anual=consumo_anual)
